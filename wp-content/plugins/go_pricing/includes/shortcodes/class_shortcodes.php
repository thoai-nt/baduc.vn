...
class GW_GoPricing_Shortcodes {
    ...
    private function __construct( $globals ) {
        ...
        /* Main shortcode */
        add_shortcode( 'go_pricing', [$this, 'pricing_table_sc'] );

        /* Video shortcodes */
        add_shortcode( 'go_pricing_html5_video', [$this, 'video_sc'] );
        add_shortcode( 'go_pricing_youtube', [$this, 'youtube_sc'] );
        add_shortcode( 'go_pricing_vimeo', [$this, 'vimeo_sc'] );
        add_shortcode( 'go_pricing_screenr', [$this, 'screenr_sc'] );
        add_shortcode( 'go_pricing_dailymotion', [$this, 'dailymotion_sc'] );
        add_shortcode( 'go_pricing_metacafe', [$this, 'metacafe_sc'] );

        /* Audio shortcodes */
        add_shortcode( 'go_pricing_audio', [$this, 'audio_sc'] );
        add_shortcode( 'go_pricing_soundcloud', [$this, 'soundcloud_sc'] );
        add_shortcode( 'go_pricing_mixcloud', [$this, 'mixcloud_sc'] );
        add_shortcode( 'go_pricing_beatport', [$this, 'beatport_sc'] );

        /* Mixed */
        add_shortcode( 'go_pricing_map', [$this, 'goole_map_sc'] );
        add_shortcode( 'go_pricing_iframe', [$this, 'iframe_sc'] );
    ...
    public function pricing_table_sc( $atts, $content = null ) {
        ...
        foreach ( $highlight as &$col_id ) {
            if ( filter_var( $col_id, FILTER_VALIDATE_INT ) === false ) {
                $col_id = null;
            }
        ...
        if ( empty( $id ) && empty( $postid ) && empty( $data ) ) {
            return '<p>' . __( 'You must set a Table ID or Post ID.', 'go_pricing_textdomain' ) . '</p>';
        }
        ...
        if ( empty( $pricing_table ) && empty( $data ) ) {
            return '<p>' . sprintf( __( 'Pricing table with an %1$s of "%2$s" is not defined.', 'go_pricing_textdomain' ), ( $id_table ) ? __( 'Table ID', 'go_pricing_textdomain' ) : __( 'Post ID', 'go_pricing_textdomain' ), !empty( $id ) ? $id : $postid ) . '</p>';
        }

        if ( empty( $pricing_table['col-data'] ) && $preview == 'true' ) {
            return '<div id="go-pricing-forbidden"><i class="icon-exclamation-triangle"></i>' . __( 'There is nothing to see here, yet. Please, first create columns!', 'go_pricing_textdomain' ) . '</div>';
        }
                    ...
                    foreach ( (array) $fonts['group_data'] as $font ) {
                        if ( !empty( $font['value'] ) && $font['value'] == $default_font ) {
                            ...
                            if ( !empty( $font['url'] ) && !empty( $font_url_params ) ) {
                                ...
                                if ( !in_array( $google_font, (array) $google_fonts ) ) {
                            ...
                            if ( !empty( $font['value'] ) ) {
                                $custom_inline_styles[] = '.gw-go { ' . sprintf( 'font-family:%s !important', $font['value'] ) . ' }';
                            }
        ...
        $all_styles = !empty( $go_pricing['styles'] ) ? $go_pricing['styles'] : [];
        ...
        foreach ( $all_styles as $style ) {
            if ( !empty( $style['class'] ) && !empty( $style['id'] ) && !empty( $current_style ) && $style['id'] == $current_style ) {
                $main_classes[] = $style['class'];
            }

        }

        if ( !empty( $pricing_table['enlarge-current'] ) ) {
            $main_classes[] = 'gw-go-enlarge-current';
        }

        if ( !empty( $pricing_table['hide-footer'] ) ) {
            $main_classes[] = 'gw-go-no-footer';
        }

        if ( !empty( $pricing_table['equalize']['fullh-bg'] ) ) {
            $main_classes[] = 'gw-go-fullh-bg';
        }

        // Box shadow
        if ( $current_style != 'classic' && empty( $pricing_table['col-box-shadow'] ) ) {
            $main_classes[] = 'gw-go-disable-box-shadow';
        }

        if ( !empty( $margin_bottom ) ) {
            $main_styles[] = sprintf( 'margin-bottom:%spx', (int) $margin_bottom );
        }
        ...
        if ( !empty( $pricing_table['style'] ) && $pricing_table['style'] == 'clean' ) {
            ...
            if ( !empty( $tooltip_style ) ) {
                $custom_inline_styles[] = sprintf( '.gw-go-tooltip-content { %s }', $tooltip_style );
            }

            if ( isset( $pricing_table['tooltip']['bg-color'] ) ) {
                $custom_inline_styles[] = '.gw-go-tooltip:before { ' . sprintf( 'border-top-color:%s;', $pricing_table['tooltip']['bg-color'] ) . ' }';
            }

            // Tooltip position
            if ( isset( $pricing_table['tooltip'] ) && isset( $pricing_table['tooltip']['position'] ) ) {
                switch ( $pricing_table['tooltip']['position'] ) {
                    case 'top-center':
                        $custom_inline_styles[] = '.gw-go-tooltip { transform: translateX(-50%); margin-left:0 }';
                        $custom_inline_styles[] = '.gw-go-tooltip-content:before { left:50%; margin-left:-6px; }';
                        break;

                    case 'top-left':
                        $custom_inline_styles[] = '.gw-go-tooltip { left:auto; right:50%; margin-left:0; margin-right:-15px; }';
                        $custom_inline_styles[] = '.gw-go-tooltip-content:before { left:auto; right:10px; }';
                        break;
                }
            }
        }

        if ( !isset( $pricing_table['col-data'] ) ) {
            return;
        }
        ...
        foreach ( (array) $pricing_table['col-data'] as $col_index => $col_data ) {
            ...
            /* Layout & Style */
            if ( isset( $col_data['layout-style'] ) && is_string( $col_data['layout-style'] ) ) {
                $col_data['layout-style'] = GW_GoPricing_Helper::parse_data( $col_data['layout-style'] );
            }

            /* Backward compatibility */
            if ( !empty( $col_data['col-highlight'] ) ) {
                $col_data['layout-style']['highlight'] = $col_data['col-highlight'];
            }

            if ( !empty( $col_data['col-disable-hover'] ) ) {
                $col_data['layout-style']['disable-hover'] = $col_data['col-disable-hover'];
            }

            if ( !empty( $col_data['col-disable-enlarge'] ) ) {
                $col_data['layout-style']['disable-enlarge'] = $col_data['col-disable-enlarge'];
            }

            if ( !empty( $col_data['main-color'] ) ) {
                $col_data['layout-style']['main-color'] = $col_data['main-color'];
            }

            // Override Highlight by shortcode param
            if ( in_array( $col_index, $highlight ) ) {
                $col_data['layout-style']['highlight'] = '1';
            }

            /* Decoration */
            if ( isset( $col_data['decoration'] ) && is_string( $col_data['decoration'] ) ) {
                $col_data['decoration'] = GW_GoPricing_Helper::parse_data( $col_data['decoration'] );
            }

            /* Header Style */
            if ( isset( $col_data['header']['style'] ) && is_string( $col_data['header']['style'] ) ) {
                $col_data['header']['style'] = GW_GoPricing_Helper::parse_data( $col_data['header']['style'] );
            }

            /* Header General */
            if ( isset( $col_data['header']['general'] ) && is_string( $col_data['header']['general'] ) ) {
                $col_data['header']['general'] = GW_GoPricing_Helper::parse_data( $col_data['header']['general'] );
            }

            /* Title */
            if ( isset( $col_data['title'] ) && is_string( $col_data['title'] ) ) {
                $col_data['title'] = GW_GoPricing_Helper::parse_data( $col_data['title'] );
            }

            /* Body row */
            if ( isset( $col_data['body-row'] ) && is_string( $col_data['body-row'] ) ) {
                $col_data['body-row'] = GW_GoPricing_Helper::parse_data( $col_data['body-row'] );
            }
                        ...
                        foreach ( $registered_style as $key => $value ) {
                            if ( $key == 'group_data' ) {
                                foreach ( (array) $value as $style_data ) {
                                    if ( !empty( $style_data['value'] ) && !empty( $style_data['data'] ) && $style_data['value'] == $column_style ) {
                                        $column_style_data = $style_data;
                                    }
...
$custom_inline_styles[] = ob_get_clean();
            ...
            if ( !empty( $column_style_data['type'] ) ) {
                if ( $column_style_data['type'] != 'html' && $column_style_data['type'] != 'chtml' ) {
                    // Title
                    if ( !empty( $col_data['title']['title']['font-size'] ) && (int) $col_data['title']['title']['font-size'] != 18 ) {
                        $title_styles[] = sprintf( 'font-size:%spx !important', (int) $col_data['title']['title']['font-size'] );
                    }

                    if ( !empty( $col_data['title']['title']['line-height'] ) && (int) $col_data['title']['title']['line-height'] != 16 ) {
                        $title_styles[] = sprintf( 'line-height:%spx !important', (int) $col_data['title']['title']['line-height'] );
                    }

                    if ( !empty( $col_data['title']['title']['font-style']['bold'] ) ) {
                        $title_styles[] = 'font-weight:bold !important';
                    }

                    if ( !empty( $col_data['title']['title']['font-style']['italic'] ) ) {
                        $title_styles[] = 'font-style:italic !important';
                    }

                    if ( !empty( $col_data['title']['title']['font-style']['strikethrough'] ) ) {
                        $title_styles[] = 'text-decoration:line-through !important';
                    }
                            ...
                            if ( !empty( $fonts['group_name'] ) && !empty( $fonts['group_data'] ) ) {
                                foreach ( (array) $fonts['group_data'] as $font ) {
                                    if ( !empty( $font['value'] ) && $font['value'] == $col_data['title']['title']['font-family'] ) {
                                        ...
                                        if ( !empty( $font['value'] ) ) {
                                            $title_styles[] = sprintf( 'font-family:%s !important', $font['value'] );
                                        }
                    ...
                    // Subtitle
                    if ( !empty( $col_data['title']['subtitle']['font-size'] ) && (int) $col_data['title']['subtitle']['font-size'] != 12 ) {
                        $title_styles[] = sprintf( 'font-size:%spx !important', (int) $col_data['title']['subtitle']['font-size'] );
                    }

                    if ( !empty( $col_data['title']['subtitle']['line-height'] ) && (int) $col_data['title']['subtitle']['line-height'] != 16 ) {
                        $title_styles[] = sprintf( 'line-height:%spx !important', (int) $col_data['title']['subtitle']['line-height'] );
                    }

                    if ( !empty( $col_data['title']['subtitle']['font-style']['bold'] ) ) {
                        $title_styles[] = 'font-weight:bold !important';
                    }

                    if ( !empty( $col_data['title']['subtitle']['font-style']['italic'] ) ) {
                        $title_styles[] = 'font-style:italic !important';
                    }

                    if ( !empty( $col_data['title']['subtitle']['font-style']['strikethrough'] ) ) {
                        $title_styles[] = 'text-decoration:line-through !important';
                    }
                    ...
                    if ( $column_style_data['type'] != 'team' && $column_style_data['type'] != 'cteam' ) {
                        if ( isset( $col_data['price'] ) && is_string( $col_data['price'] ) ) {
                            $col_data['price'] = GW_GoPricing_Helper::parse_data( $col_data['price'] );
                        }

                        // Price
                        if ( !empty( $col_data['price']['price-style']['font-size'] ) && (int) $col_data['price']['price-style']['font-size'] != 32 ) {
                            $price_styles[] = sprintf( 'font-size:%spx !important', (int) $col_data['price']['price-style']['font-size'] );
                        }

                        if ( !empty( $col_data['price']['price-style']['line-height'] ) && (int) $col_data['price']['price-style']['line-height'] != 16 ) {
                            $price_styles[] = sprintf( 'line-height:%spx !important', (int) $col_data['price']['price-style']['line-height'] );
                        }

                        if ( !empty( $col_data['price']['price-style']['font-style']['bold'] ) ) {
                            $price_styles[] = 'font-weight:bold !important';
                        }

                        if ( !empty( $col_data['price']['price-style']['font-style']['italic'] ) ) {
                            $price_styles[] = 'font-style:italic !important';
                        }

                        if ( !empty( $col_data['price']['price-style']['font-style']['strikethrough'] ) ) {
                            $price_styles[] = 'text-decoration:line-through !important';
                        }
                                ...
                                if ( !empty( $fonts['group_name'] ) && !empty( $fonts['group_data'] ) ) {
                                    foreach ( (array) $fonts['group_data'] as $font ) {
                                        if ( !empty( $font['value'] ) && $font['value'] == $col_data['price']['price-style']['font-family'] ) {
                                            ...
                                            if ( !empty( $font['value'] ) ) {
                                                $price_styles[] = sprintf( 'font-family:%s !important', $font['value'] );
                                            }
                        ...
                        // Payment
                        if ( !empty( $col_data['price']['payment']['font-size'] ) && (int) $col_data['price']['payment']['font-size'] != 12 ) {
                            $payment_styles[] = sprintf( 'font-size:%spx !important', (int) $col_data['price']['payment']['font-size'] );
                        }

                        if ( !empty( $col_data['price']['payment']['line-height'] ) && (int) $col_data['price']['payment']['line-height'] != 16 ) {
                            $payment_styles[] = sprintf( 'line-height:%spx !important', (int) $col_data['price']['payment']['line-height'] );
                        }

                        if ( !empty( $col_data['price']['payment']['font-style']['bold'] ) ) {
                            $payment_styles[] = 'font-weight:bold !important';
                        }

                        if ( !empty( $col_data['price']['payment']['font-style']['italic'] ) ) {
                            $payment_styles[] = 'font-style:italic !important';
                        }

                        if ( !empty( $col_data['price']['payment']['font-style']['strikethrough'] ) ) {
                            $payment_styles[] = 'text-decoration:line-through !important';
                        }
                                ...
                                if ( !empty( $fonts['group_name'] ) && !empty( $fonts['group_data'] ) ) {
                                    foreach ( (array) $fonts['group_data'] as $font ) {
                                        if ( !empty( $font['value'] ) && $font['value'] == $col_data['price']['payment']['font-family'] ) {
                                            ...
                                            if ( !empty( $font['value'] ) ) {
                                                $payment_styles[] = sprintf( 'font-family:%s !important', $font['value'] );
                                            }
            ...
            if ( !empty( $col_data['layout-style']['highlight'] ) ) {
                $col_wrap_classes[] = 'gw-go-hover';
            }

            if ( !empty( $col_data['layout-style']['highlight'] ) ) {
                $col_wrap_atts[] = ['data-current', true];
            }

            if ( !empty( $col_data['layout-style']['disable-enlarge'] ) ) {
                $col_wrap_classes[] = 'gw-go-disable-enlarge';
            }

            if ( !empty( $col_data['layout-style']['disable-hover'] ) ) {
                $col_wrap_classes[] = 'gw-go-disable-hover';
            }

            $col_wrap_atts[] = ['data-col-index', $col_index];
            ...
            foreach ( (array) $col_wrap_atts as $col_wrap_att ) {
                if ( count( (array) $col_wrap_att ) == 2 ) {
                    $col_wrap_attributes[] = sprintf( '%1$s="%2$s"', $col_wrap_att[0], GW_GoPricing_Helper::esc_attr( $col_wrap_att[1] ) );
                }
            ...
            if ( isset( $go_pricing['column-transition'] ) && !empty( $pricing_table['col-animation'] ) ) {
                if ( !empty( $col_data['animation'] ) && is_string( $col_data['animation'] ) ) {
                    $col_anim = GW_GoPricing_Helper::parse_data( $col_data['animation'] );
                }
                        ...
                        if ( !empty( $trans['group_name'] ) && !empty( $trans['group_data'] ) ) {
                            foreach ( $trans['group_data'] as $trans_child ) {
                                if ( isset( $trans_child['value'] ) && $trans_child['value'] == $col_anim['transition'] ) {
                                    if ( isset( $trans_child['data'] ) ) {
                                        $col_anim_data['css'] = json_decode( $trans_child['data'], true );
                                    }
                            ...
                            if ( isset( $trans['value'] ) && $trans['value'] == $col_anim['transition'] ) {
                                if ( isset( $trans['data'] ) ) {
                                    $col_anim_data['css'] = json_decode( $trans['data'], true );
                                }
            ...
            if ( !empty( $col_data['decoration']['col-shadow'] ) ) {
                $col_classes[] = 'gw-go-' . $col_data['decoration']['col-shadow'];
            }
            ...
            if ( !empty( $col_data['decoration']['col-sign'] ) && !empty( $col_data['decoration']['col-sign-type'] ) ) {
                ...
                if ( $col_data['decoration']['col-sign-type'] == 'text' ) {
                    ...
                    if ( !empty( $ribbon_data['shadow'] ) ) {
                        $sign_classes[] = 'gw-go-ribbon-shadow';
                    }
                    ...
                    if ( !empty( $ribbon_data['font-size'] ) && (int) $ribbon_data['font-size'] != 12 ) {
                        $sign_span_styles[] = sprintf( 'font-size:%spx !important', (int) $ribbon_data['font-size'] );
                    }

                    if ( !empty( $ribbon_data['font-style']['bold'] ) ) {
                        $sign_span_styles[] = 'font-weight:bold !important';
                    }

                    if ( !empty( $ribbon_data['font-style']['italic'] ) ) {
                        $sign_span_styles[] = 'font-style:italic !important';
                    }

                    if ( !empty( $ribbon_data['font-style']['strikethrough'] ) ) {
                        $sign_span_styles[] = 'text-decoration:line-through !important';
                    }
                            ...
                            if ( !empty( $fonts['group_name'] ) && !empty( $fonts['group_data'] ) ) {
                                foreach ( (array) $fonts['group_data'] as $font ) {
                                    if ( !empty( $font['value'] ) && $font['value'] == $ribbon_data['font-family'] ) {
                                        ...
                                        if ( !empty( $font['value'] ) ) {
                                            $sign_span_styles[] = sprintf( 'font-family:%s !important', $font['value'] );
                                        }
                ...
                if ( !empty( $col_data['decoration']['col-sign-align'] ) ) {
                    $sign_classes[] = $col_data['decoration']['col-sign-align'] == 'left' ? 'gw-go-ribbon-left' : 'gw-go-ribbon-right';
                }

                if ( !empty( $sign_inner_html ) && !empty( $col_data['decoration']['col-sign-link']['url'] ) ) {
                    if ( !empty( $col_data['decoration']['col-sign-link']['target'] ) ) {
                        $sign_link_atts[] = ['target', '_blank'];
                    }

                    if ( !empty( $col_data['decoration']['col-sign-link']['nofollow'] ) ) {
                        $sign_link_atts[] = ['rel', 'nofollow'];
                    }
                    ...
                    foreach ( (array) $sign_link_atts as $sign_link_att ) {
                        if ( count( (array) $sign_link_att ) == 2 ) {
                            $sign_link_attributes[] = sprintf( '%1$s="%2$s"', $sign_link_att[0], GW_GoPricing_Helper::esc_attr( $sign_link_att[1] ) );
                        }
                ...
                if ( !empty( $col_data['decoration']['col-sign-position']['posy'] ) ) {
                    $sign_styles[] = sprintf( 'top:%dpx', (int) $col_data['decoration']['col-sign-position']['posy'] );
                }
            ...
            if ( !empty( $column_style_data['type'] ) ) {
                ...
                // Build html
                if ( !empty( $col_data['header']['general']['replace'] ) ) {
                    $column_style_data['type'] = 'html';
                }
                ...
                switch ( $column_style_data['type'] ) {
                        ...
                        switch ( $price_type ) {
                            ...
                            case 'price':
                                ...
                                foreach ( (array) $go_pricing['currency'] as $currency ) {
                                    if ( !empty( $currency['id'] ) && !empty( $currency['symbol'] ) && !empty( $general_settings['currency'][0]['currency'] ) && $currency['id'] == $general_settings['currency'][0]['currency'] ) {
                                        $currency_symbol = $currency['symbol'];
                                    }

                                    if ( isset( $general_settings['currency'][0]['symbol'] ) && trim( $general_settings['currency'][0]['symbol'] ) !== '' ) {
                                        $currency_symbol = trim( $general_settings['currency'][0]['symbol'] );
                                    }
                    ...
                    case 'cpricing2':
                        ...
                        foreach ( (array) $header_custom_styles as $key => $header_custom_style ) {
                            if ( !empty( $header_custom_styles[$key] ) ) {
                                $header_custom_styles[$key] = trim( $header_custom_style );
                            }
                    ...
                    case 'chtml':
                        ...
                        foreach ( (array) $header_styles as $key => $header_style ) {
                            if ( !empty( $header_styles[$key] ) ) {
                                $header_styles[$key] = trim( $header_style );
                            }
            ...
            if ( isset( $table_data['col-data'] ) ) {
                unset( $table_data['col-data'] );
            }
            ...
            if ( is_array( $header_data ) && !empty( $header_data['css'] ) ) {
                $custom_inline_styles = array_merge( $custom_inline_styles, $header_data['css'] );
            }

            if ( is_array( $header_data ) && !empty( $header_data['google_fonts'] ) ) {
                $google_fonts = $header_data['google_fonts'];
            }
            ...
            if ( isset( $pricing_table['col-data'][$col_index]['body-row'] ) ) {
                foreach ( (array) $pricing_table['col-data'][$col_index]['body-row'] as $body_row_index => $body_row_data ) {
                    if ( is_string( $body_row_data ) ) {
                        $body_row_data = GW_GoPricing_Helper::parse_data( $body_row_data );
                    }
                    ...
                    if ( isset( $pricing_table['tooltip']['width'] ) ) {
                        $tooltip_styles[] = sprintf( 'width:%dpx', (int) $pricing_table['tooltip']['width'] );
                    }

                    if ( !empty( $pricing_table['tooltip']['text-color'] ) ) {
                        $tooltip_styles[] = 'color:' . $pricing_table['tooltip']['text-color'];
                    }

                    if ( !empty( $pricing_table['tooltip']['bg-color'] ) ) {
                        $tooltip_styles[] = 'background-color:' . $pricing_table['tooltip']['bg-color'];
                    }

                    if ( !empty( $pricing_table['tooltip']['bg-color'] ) ) {
                        $tooltip_styles[] = 'border-color:' . $pricing_table['tooltip']['bg-color'];
                    }

                    if ( isset( $body_row_data['type'] ) && !empty( $body_row_data[$body_row_data['type']]['tooltip']['font-size'] ) ) {
                        $tooltip_styles[] = sprintf( 'font-size:%spx !important', (int) $body_row_data[$body_row_data['type']]['tooltip']['font-size'] );
                    }

                    if ( isset( $body_row_data['type'] ) && !empty( $body_row_data[$body_row_data['type']]['tooltip']['line-height'] ) ) {
                        $tooltip_styles[] = sprintf( 'line-height:%spx !important', (int) $body_row_data[$body_row_data['type']]['tooltip']['line-height'] );
                    }

                    if ( isset( $body_row_data['type'] ) && !empty( $body_row_data[$body_row_data['type']]['tooltip']['font-style']['bold'] ) ) {
                        $tooltip_styles[] = 'font-weight:bold !important';
                    }

                    if ( isset( $body_row_data['type'] ) && !empty( $body_row_data[$body_row_data['type']]['tooltip']['font-style']['italic'] ) ) {
                        $tooltip_styles[] = 'font-style:italic !important';
                    }

                    if ( isset( $body_row_data['type'] ) && !empty( $body_row_data[$body_row_data['type']]['tooltip']['font-style']['strikethrough'] ) ) {
                        $tooltip_styles[] = 'text-decoration:line-through !important';
                    }
                                ...
                                foreach ( (array) $fonts['group_data'] as $font ) {
                                    if ( !empty( $font['value'] ) && $font['value'] == $body_row_data[$body_row_data['type']]['tooltip']['font-family'] ) {
                                        if ( !empty( $font['name'] ) && !empty( $font['url'] ) ) {
                                            ...
                                            if ( preg_match( '/fonts.googleapis.com/', $font['url'] ) ) {
                                                $font_url_params[] = '400';
                                                if ( !empty( $body_row_data[$body_row_data['type']]['tooltip']['font-style']['bold'] ) ) {
                                                    $font_url_params[] = 'b';
                                                }

                                                if ( !empty( $body_row_data[$body_row_data['type']]['tooltip']['font-style']['italic'] ) ) {
                                                    $font_url_params[] = 'i';
                                                }
                                        ...
                                        if ( !empty( $font['value'] ) ) {
                                            $tooltip_styles[] = sprintf( 'font-family:%s !important', $font['value'] );
                                        }
                    ...
                    if ( !empty( $body_row_data['type'] ) ) {
                        ...
                        if ( !empty( $body_row_data['row-align'] ) ) {
                            if ( $body_row_data['type'] == 'button' ) {
                                $body_row_data['row-align'] = 'center';
                            }
                        ...
                        if ( $body_row_data['type'] == 'html' ) {
                            if ( !empty( $body_row_data['html']['font-size'] ) ) {
                                $body_row_inline_styles[] = sprintf( 'font-size:%spx !important', (int) $body_row_data['html']['font-size'] );
                            }

                            if ( !empty( $body_row_data['html']['line-height'] ) ) {
                                $body_row_inline_styles[] = sprintf( 'line-height:%spx !important', (int) $body_row_data['html']['line-height'] );
                            }

                            if ( !empty( $body_row_data['html']['text-align'] ) ) {
                                $body_row_inline_styles[] = sprintf( 'text-align:%s !important', $body_row_data['html']['text-align'] );
                            }

                            if ( !empty( $body_row_data['html']['font-style']['bold'] ) ) {
                                $body_row_inline_styles[] = 'font-weight:bold !important';
                            }

                            if ( !empty( $body_row_data['html']['font-style']['italic'] ) ) {
                                $body_row_inline_styles[] = 'font-style:italic !important';
                            }

                            if ( !empty( $body_row_data['html']['font-style']['strikethrough'] ) ) {
                                $body_row_inline_styles[] = 'text-decoration:line-through !important';
                            }
                                    ...
                                    if ( !empty( $fonts['group_name'] ) && !empty( $fonts['group_data'] ) ) {
                                        foreach ( (array) $fonts['group_data'] as $font ) {
                                            if ( !empty( $font['value'] ) && $font['value'] == $body_row_data['html']['font-family'] ) {
                                                ...
                                                if ( !empty( $font['value'] ) ) {
                                                    $body_row_inline_styles[] = sprintf( 'font-family:%s !important', $font['value'] );
                                                }
                        ...
                        if ( $body_row_data['type'] == 'button' ) {
                            if ( !empty( $body_row_data['button']['font-size'] ) ) {
                                $body_row_inline_styles[] = sprintf( 'font-size:%spx !important', (int) $body_row_data['button']['font-size'] );
                            }

                            if ( !empty( $body_row_data['button']['font-style']['bold'] ) ) {
                                $body_row_inline_styles[] = 'font-weight:bold !important';
                            }

                            if ( !empty( $body_row_data['button']['font-style']['italic'] ) ) {
                                $body_row_inline_styles[] = 'font-style:italic !important';
                            }

                            if ( !empty( $body_row_data['button']['font-style']['strikethrough'] ) ) {
                                $body_row_inline_styles[] = 'text-decoration:line-through !important';
                            }
                        ...
                        if ( !empty( $body_row_inline_styles ) && is_array( $body_row_inline_styles ) ) {
                            $custom_inline_styles = array_merge( $custom_inline_styles, $body_row_inline_styles );
                        }
                        ...
                        switch ( $body_row_data['type'] ) {
                            ...
                            case 'button':
                                ...
                                $button_data = !empty( $body_row_data['button'] ) ? $body_row_data['button'] : [];
                                $button_css = [];
                                ...
                                if ( !empty( $body_row_data[$body_row_data['type']]['size'] ) ) {
                                    $button_classes[] = 'gw-go-btn-' . $body_row_data[$body_row_data['type']]['size'];
                                }

                                if ( !empty( $body_row_data[$body_row_data['type']]['target'] ) ) {
                                    $button_atts[] = ['target', '_blank'];
                                }

                                if ( !empty( $body_row_data[$body_row_data['type']]['nofollow'] ) ) {
                                    $button_atts[] = ['rel', 'nofollow'];
                                }

                                if ( $current_style == 'clean' ) {
                                    if ( !empty( $button_data['bg-color'] ) ) {
                                        $button_css['normal'][] = sprintf( 'background-color:%s !important', $button_data['bg-color'] );
                                    }

                                    if ( !empty( $button_data['color'] ) ) {
                                        $button_css['normal'][] = sprintf( 'color:%s !important', $button_data['color'] );
                                    }

                                    if ( !empty( $button_css['normal'] ) ) {
                                        $custom_inline_styles[] = sprintf( '.gw-go-col-wrap-%1$s .gw-go-body li[data-row-index="%2$s"] .gw-go-btn { %3$s; }',
                                            $col_index,
                                            $body_row_index,
                                    ...
                                    if ( !empty( $button_data['bg-color-hover'] ) ) {
                                        $button_css['hover'][] = sprintf( 'background-color:%s !important', $button_data['bg-color-hover'] );
                                    }

                                    if ( !empty( $button_data['color-hover'] ) ) {
                                        $button_css['hover'][] = sprintf( 'color:%s  !important', $button_data['color-hover'] );
                                    }

                                    if ( !empty( $button_css['hover'] ) ) {
                                        $custom_inline_styles[] = sprintf( '.gw-go-hover.gw-go-col-wrap-%1$s .gw-go-body li[data-row-index="%2$s"] .gw-go-btn { %3$s; }',
                                            $col_index,
                                            $body_row_index,
                                ...
                                foreach ( (array) $button_atts as $button_att ) {
                                    if ( count( (array) $button_att ) == 2 ) {
                                        $button_attributes[] = sprintf( '%1$s="%2$s"', $button_att[0], GW_GoPricing_Helper::esc_attr( $button_att[1] ) );
                                    }
            ...
            if ( empty( $pricing_table['hide-footer'] ) && !empty( $pricing_table['col-data'][$col_index]['footer-row'] ) ) {
                foreach ( (array) $pricing_table['col-data'][$col_index]['footer-row'] as $footer_row_index => $footer_row_data ) {
                    if ( is_string( $footer_row_data ) ) {
                        $footer_row_data = GW_GoPricing_Helper::parse_data( $footer_row_data );
                    }
                    ...
                    // Deprecated
                    if ( isset( $pricing_table['tooltip-width'] ) ) {
                        $tooltip_styles[] = 'width:' . $pricing_table['tooltip-width'];
                    }
                    ...
                    if ( !empty( $footer_row_data['type'] ) ) {
                        ...
                        if ( !empty( $footer_row_data['row-align'] ) ) {
                            if ( $footer_row_data['type'] == 'button' ) {
                                $footer_row_data['row-align'] = 'center';
                            }
                        ...
                        if ( $footer_row_data['type'] == 'html' ) {
                            if ( !empty( $footer_row_data['html']['font-size'] ) ) {
                                $footer_row_inline_styles[] = sprintf( 'font-size:%spx !important', (int) $footer_row_data['html']['font-size'] );
                            }

                            if ( !empty( $footer_row_data['html']['line-height'] ) ) {
                                $footer_row_inline_styles[] = sprintf( 'line-height:%spx !important', (int) $footer_row_data['html']['line-height'] );
                            }

                            if ( !empty( $footer_row_data['html']['text-align'] ) ) {
                                $footer_row_inline_styles[] = sprintf( 'text-align:%s !important', $footer_row_data['html']['text-align'] );
                            }

                            if ( !empty( $footer_row_data['html']['font-style']['bold'] ) ) {
                                $footer_row_inline_styles[] = 'font-weight:bold !important';
                            }

                            if ( !empty( $footer_row_data['html']['font-style']['italic'] ) ) {
                                $footer_row_inline_styles[] = 'font-style:italic !important';
                            }

                            if ( !empty( $footer_row_data['html']['font-style']['strikethrough'] ) ) {
                                $footer_row_inline_styles[] = 'text-decoration:line-through !important';
                            }
                                    ...
                                    if ( !empty( $fonts['group_name'] ) && !empty( $fonts['group_data'] ) ) {
                                        foreach ( (array) $fonts['group_data'] as $font ) {
                                            if ( !empty( $font['value'] ) && $font['value'] == $footer_row_data['html']['font-family'] ) {
                                                ...
                                                if ( !empty( $font['value'] ) ) {
                                                    $footer_row_inline_styles[] = sprintf( 'font-family:%s !important', $font['value'] );
                                                }
                        ...
                        if ( $footer_row_data['type'] == 'button' ) {
                            if ( !empty( $footer_row_data['button']['font-size'] ) ) {
                                $footer_row_inline_styles[] = sprintf( 'font-size:%spx !important', (int) $footer_row_data['button']['font-size'] );
                            }

                            if ( !empty( $footer_row_data['button']['font-style']['bold'] ) ) {
                                $footer_row_inline_styles[] = 'font-weight:bold !important';
                            }

                            if ( !empty( $footer_row_data['button']['font-style']['italic'] ) ) {
                                $footer_row_inline_styles[] = 'font-style:italic !important';
                            }

                            if ( !empty( $footer_row_data['button']['font-style']['strikethrough'] ) ) {
                                $footer_row_inline_styles[] = 'text-decoration:line-through !important';
                            }
                        ...
                        if ( !empty( $footer_row_inline_styles ) ) {
                            $custom_inline_styles = array_merge( $custom_inline_styles, $footer_row_inline_styles );
                        }
                        ...
                        switch ( $footer_row_data['type'] ) {
                            ...
                            case 'button':
                                ...
                                $button_data = !empty( $footer_row_data['button'] ) ? $footer_row_data['button'] : [];
                                $button_css = [];
                                ...
                                $button_wrap_classes[] = 'gw-go-btn-wrap';
                                $button_classes[] = 'gw-go-btn';
                                if ( !empty( $button_data['size'] ) ) {
                                    $button_classes[] = 'gw-go-btn-' . $button_data['size'];
                                }

                                if ( !empty( $button_data['target'] ) ) {
                                    $button_atts[] = ['target', '_blank'];
                                }

                                if ( !empty( $button_data['nofollow'] ) ) {
                                    $button_atts[] = ['rel', 'nofollow'];
                                }
                                ...
                                if ( $current_style == 'clean' ) {
                                    ...
                                    if ( !empty( $button_css['normal'] ) ) {
                                        $custom_inline_styles[] = sprintf( '.gw-go-col-wrap-%1$s .gw-go-footer-row[data-row-index="%2$s"] .gw-go-btn { %3$s; }',
                                            $col_index,
                                            $footer_row_index,
                                            !empty( $button_css['normal'] ) ? implode( '; ', $button_css['normal'] ) : ''
                                        );
                                    ...
                                    if ( !empty( $button_css['hover'] ) ) {
                                        $custom_inline_styles[] = sprintf( '.gw-go-hover.gw-go-col-wrap-%1$s .gw-go-footer-row[data-row-index="%2$s"] .gw-go-btn { %3$s; }',
                                            $col_index,
                                            $footer_row_index,
                                            !empty( $button_css['hover'] ) ? implode( '; ', $button_css['hover'] ) : ''
                                        );
        ...
        /* Import icon fonts */
        if ( preg_match( '/<i[^<>]*class=(\"|\')(.*)?fa(?:(<\/i>).*)?(\2)/', $html ) && empty( $general_settings['disable-font']['fa'] ) ) {
            $font_css .= sprintf( '@import url(%s);', $this->plugin_url . 'assets/lib/font_awesome/css/font-awesome.min.css' );
        }

        if ( preg_match( '/<i[^<>]*class=(\"|\')(.*)?linecon-(?:(<\/i>).*)?(\2)/', $html ) && empty( $general_settings['disable-font']['linecon'] ) ) {
            $font_css .= sprintf( '@import url(%s);', $this->plugin_url . 'assets/lib/linecon/linecon.min.css' );
        }

        if ( preg_match( '/<i[^<>]*class=(\"|\')(.*)?icomoon-(?:(<\/i>).*)?(\2)/', $html ) && empty( $general_settings['disable-font']['icomoon'] ) ) {
            $font_css .= sprintf( '@import url(%s);', $this->plugin_url . 'assets/lib/icomoon/icomoon.min.css' );
        }

        if ( preg_match( '/<i[^<>]*class=(\"|\')(.*)?material-(?:(<\/i>).*)?(\2)/', $html ) && empty( $general_settings['disable-font']['material'] ) ) {
            $font_css .= sprintf( '@import url(%s);', $this->plugin_url . 'assets/lib/material/material.min.css' );
        }
            ...
            if ( isset( $pricing_table['responsivity']['views'] ) ) {
                foreach ( $pricing_table['responsivity']['views'] as $view ) {
                    ...
                    if ( isset( $view['min'] ) && $view['min'] != '' ) {
                        $mq[] = sprintf( '(min-width: %dpx)', (int) $view['min'] );
                    }

                    if ( isset( $view['max'] ) && $view['max'] != '' ) {
                        $mq[] = sprintf( '(max-width: %dpx)', (int) $view['max'] );
                    }
                    ...
                    if ( !empty( $mq ) ) {
                        ...
                        // Tooltips alignment on mobiles and tablets
                        $mq_css .= sprintf(
                            '<style>@media only screen and %1$s { %2$s }</style>',
                            implode( ' and ', $mq ),
                            '.gw-go-tooltip { left:50% !important; right:auto !important; transform: translateX(-50%) !important; margin-left:0!important; } .gw-go-tooltip-content:before { right:auto !important; left:50% !important; margin-left:-6px !important; }'
                        );
...
<img src="' . $poster_src . '" title="No video playback capabilities" />
    ...
    public function goole_map_sc( $atts, $content = null ) {
        ...
        $mapdata['markers'][] = [
            'address' => $address,
            'title' => $title,
            'icon' => !empty( $icon ) ? ['image' => $icon] : null,
            'html' => isset( $content ) ? [
                'content' => $content,
                'popup' => $popup
            ] : null
